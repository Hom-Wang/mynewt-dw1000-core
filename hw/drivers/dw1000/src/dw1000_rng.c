...
dw1000_rng_request(dw1000_dev_instance_t * inst, uint16_t dst_address, dw1000_rng_modes_t code){
    ...
    DIAGMSG("{\"utime\": %lu,\"msg\": \"dw1000_rng_request\"}\n",os_cputime_ticks_to_usecs(os_cputime_get32()));
    ...
    switch(frame->code){
        case DWT_SS_TWR ... DWT_SS_TWR_END:
            ToF = ((frame->response_timestamp - frame->request_timestamp) 
                    -  (frame->transmission_timestamp - frame->reception_timestamp))/2.; 
        break;
        case DWT_DS_TWR ... DWT_DS_TWR_END:
        case DWT_DS_TWR_EXT ... DWT_DS_TWR_EXT_END:
            T1R = (first_frame->response_timestamp - first_frame->request_timestamp); 
...
rng_tx_complete_cb(dw1000_dev_instance_t * inst)
{
    ...
    if (inst->fctrl == FCNTL_IEEE_RANGE_16){
        ...
        if (frame->code == DWT_SS_TWR_FINAL || frame->code ==  DWT_DS_TWR_FINAL || frame->code ==  DWT_DS_TWR_EXT_FINAL){
            os_sem_release(&inst->rng->sem);  
        } 
    }
                ...
                case  DWT_SS_TWR_FINAL:
                    {
                        ...
                        // This marks the completion of the single-size-two-way request. This final 4th message is optional in some applicaiton. 
